...
- `{voice}` 代入聲線鍵,`{n}` 代入幕的 **1-based scene 編號**。例如 `default=yunjhe`、第一幕(n=1)→ `assets/narration/yunjhe/scene1.mp3`;切到 `hsiaochen` 第七幕(n=7)→ `assets/narration/hsiaochen/scene7.mp3`。引擎介面上可切換聲線(在 `voices` 之間輪替)。
- `cues` 指向的 `cues.json` 是**字幕資料**(每幕、每聲線的句子時間與文字),由旁白資產自帶。manifest 只負責**指到它**;字幕內容不在 `audio.json` 裡編。
